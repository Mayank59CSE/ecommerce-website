...
def Add_To_Cart(request):
        try:
            ...
            # create cart container using Session Management
            try:
                Cart_Container = request.session.get('Cart_Container', {})
                Cart_Container[str(product['productid'])] = product
                request.session['Cart_Container'] = Cart_Container
            ...
            print("Cart_Container:", Cart_Container)
            Cart_Container_str = json.dumps(Cart_Container)
            return JsonResponse({'data': Cart_Container_str}, safe=False)
...
def Fetch_Cart(request):
    try:
     ...
     return JsonResponse({'data': Cart_Container}, safe=False)
        ...
        print("Fetch_Cart:",err)
        return JsonResponse({'data': []}, safe=False)
def RemoveFromCart(request, totalprice=None):
    # try:
    #     productid = int(request.GET['productid'])  # Convert productid to an integer
    #
    #     Cart_Container = request.session.get('Cart_Container', {})  # Use get with a default empty dictionary
    #     if productid in Cart_Container:
...
def Shopping_Cart(request):
    ...
    #         totalsavings = 0
    #
    #         for index, key in enumerate(Cart_Container.keys(), start=1):
    ...
    #            Cart_Container[key]['productprice'] = int(Cart_Container[key]['offerprice']) * int(
    ...
    #         # Update the key to use natural numbers
    #         Cart_Container[index] = Cart_Container.pop(key)
    #     except Exception as err:
    #         Cart_Container = {}
